...
function Cart() {
  ...
  const total = items.reduce((acc, item) => acc + parseFloat(item.total_amount), 0);

  return (
    <>
      <Navbar />

      <div className="container" style={{ padding: "60px 0", minHeight: "70vh" }}>
        <h2 style={{ fontSize: "2.5rem", marginBottom: "40px" }}>{t('shopping_cart')}</h2>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 350px", gap: "40px", alignItems: "start" }}>
                ...
                <tbody>
                  {items.map((item) => (
                    <tr key={item.id} style={{ borderBottom: "1px solid #eee" }}>
                      <td style={{ padding: "20px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "15px" }}>
                          <img
                            ...
                            onError={(event) => {
                              event.currentTarget.src = getFallbackProductImage(item.product_details);
                            }}
                            style={{
                              width: "50px",
                              height: "50px",
                          ...
                          <div>
                            <div style={{ fontWeight: "600" }}>
                              {item.product_details?.name || `${t('product_header')} #${item.product}`}
                            </div>
                      ...
                      <td style={{ padding: "20px", fontWeight: "600" }}>₹{item.total_amount}</td>
            ...
            <div className="card" style={{ position: "sticky", top: "100px" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "15px" }}>
                ...
                <span style={{ color: "var(--primary-green)", fontWeight: "bold" }}>{t('free')}</span>
              </div>
              ...
              <p style={{ textAlign: "center", fontSize: "12px", color: "var(--text-light)", marginTop: "15px" }}>
                {t('secure_payments')}
              </p>
            </div>
          </div>
