...
export default function ProductCard({ product }) {
  ...
  const addToCart = async () => {
    ...
    if (!token) {
      alert(t('login_required_alert') || "Please login to add products to cart");
      return;
    }
  ...
  const getCategoryLabel = (cat) => {
    switch(cat?.toLowerCase()) {
      case 'vegetables': return t('fresh_veg');
      case 'fruits': return t('organic_fruits');
      case 'dairy': return t('dairy_eggs');
      case 'grains': return t('grains_pulses');
      default: return cat || t('fresh_veg');
    }
  };
      ...
      <div style={{ padding: "20px" }}>
        ...
        >
          ...
          <button
            ...
            disabled={loading}
            style={{
              background: message ? "var(--accent-orange)" : "var(--primary-green)",
              color: "white",
              padding: "10px 15px",
              borderRadius: "8px",
              fontSize: "14px",
